  ...
  ?>
    ...
    <article style="border:0px" class="blog">
        ...
        <div class="blog-right">
            ...
            <p 
             <?php if(!$this->session->userdata('comuser_name')&&!$this->session->userdata('logged_in')) {?>
	  ...
	  ?>
        ...
        <div style="display:none" class="commentrespond comrdesc comrply<?php echo $i; ?>">
            <form action="" id="replysubmit<?php echo $i;?>"  method="post">
                ...
                --><input id="author" name="reply_name" class="reply_name" type="hidden" value="<?php echo $sesval['comuser_name'];  ?><?php echo $user_log['username']?>" size="30" aria-required="true">
                </p>
                
                <p class="comment-form-email">
                <!--<label for="email">Email <span class="required">*</span></label> <br />
                --><input id="email" name="reply_email"  class="reply_email" type="hidden" value="<?php echo $sesval['comuser_email']; ?><?php echo $user_log['email']?>" size="30" aria-required="true">
                </p>
...
<div class="commentrespond">
 ...
 //$this->session->sess_destroy()
  ...
  if(!$sesval['comuser_email']&&!$user_log['email']){ 			
	  ?>
 <h3 class="comment-reply-title">To Reply on this topic choose an option below and remain anymous</h3> <?php }?>
...
<div class="firstsection">
   <?php if(!$sesval['comuser_email']&&!$user_log['email']){ ?>
    <p class="comment-form-author">
        <label for="author">Username <span class="required">*</span></label><br /> </p>
		<p style="color:#F00" class="errorname"></p>
		<?php }?>
        <input id="fcomment_name" required="required" name="fcomment_name" <?php if($sesval['comuser_email']||$user_log['email']){ echo 'type="hidden"';}else{?> type="text"<?php }?> value="<?php echo $sesval['comuser_name'];  ?><?php echo $user_log['username'];  ?>" size="30" aria-required="true">
   
  <?php if(!$sesval['comuser_email']&&!$user_log['email']){ ?>     
    <p class="comment-form-email">
        <label for="email">Email <span class="required">*</span></label> <br /> </p>
		<p style="color:#F00" class="errorall erroremail"></p>
		<?php }?>
        <input id="fcomment_email" name="fcomment_email"  <?php if($sesval['comuser_email']||$user_log['email']){ echo 'type="hidden"';}else{?> type="text"<?php }?> value="<?php echo $sesval['comuser_email'];  ?><?php echo $user_log['email'];  ?>" size="30" aria-required="true">
         <?php if(!$sesval['comuser_email']&&!$user_log['email']){ ?> <div class="text-or"><span style="padding-right:5px; font-weight:bold;font-size:17px; ">OR </span> <a style="font-weight:bold; color:#57e2ca" href="<?php echo JEWISH_URL;?>/login/redirect/?redirect=<?php echo 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']?>"">Login</a> to your account and reply on the topic and track your comment and enable comment editing</div> <?php }?>
     
   <input type="hidden" name="forum_id" value="<?php echo $forumdata['forum_id']; ?>" /> 
   <?php if(!$sesval['comuser_email']&&!$user_log['email']){?> <div class="checking">Go</div><?php } ?>
   </div>
